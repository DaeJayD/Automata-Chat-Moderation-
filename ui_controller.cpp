...
void ChatModerationUI::analyze_toxic_phrases() {
    ...
    cout << "Enter regex pattern to match toxic phrases (e.g., \"(fuck|yawa|animal) \"):\n";
    cout << YELLOW << ">> " << RESET;
    ...
    try {
        ...
        cout << "Enter message to analyze (or press Enter for default test):\n";
        cout << YELLOW << ">> " << RESET;
        string message;
        getline(cin, message);
        
        if (message.empty()) {
            message = "This contains fuck and animal but not yawa";
            cout << YELLOW << "Using default test message: \"" << message << "\"\n" << RESET;
        }
